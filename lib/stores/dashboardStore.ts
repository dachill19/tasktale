    ...
    return {
        ...
        isTaskOverdue: (dueDate: string): boolean => {
            try {
                ...
                if (!taskDate.isValid) {
                    console.warn(
                        `Invalid dueDate in isTaskOverdue: ${dueDate}`
                    );
                    return false;
                }
        ...
        isTaskUpcoming: (dueDate: string): boolean => {
            try {
                ...
                if (!taskDate.isValid) {
                    console.warn(
                        `Invalid dueDate in isTaskUpcoming: ${dueDate}`
                    );
                    return true;
                }
        ...
        fetchData: async () => {
            ...
            try {
                ...
                if (!userResponse.success || !userResponse.data) {
                    set({
                        error: userResponse.error || "Failed to get user ID",
                    });
                    return;
                }
                ...
                if (todayTaskResponse.success && todayTaskResponse.data) {
                    const transformedTodayTasks = todayTaskResponse.data.map(
                        (task) => {
                            const transformed = transformTaskForCard(task);
                            const formattedDueDate = get().safeFormatDate(
                                transformed.originalDeadline
                            );
                            return {
                                id: transformed.id,
                    ...
                    console.warn(
                        "Today task fetch warning:",
                        todayTaskResponse.error
                    );
                    set({ todayTasks: [] });
                }
                ...
                if (allTaskResponse.success && allTaskResponse.data) {
                    ...
                    console.warn(
                        "All task fetch warning:",
                        allTaskResponse.error
                    );
                    set({ allTasks: [] });
                }
                ...
                if (journalResponse.success && journalResponse.data) {
                    const transformedJournals = journalResponse.data.map(
                        (journal) => {
                            const transformed =
                                transformJournalForCard(journal);
                            const formattedDate = get().safeFormatDate(
