...
interface TaskState {
  ...
  createTask: (taskData: TaskFormData, userId: string) => Promise<ServiceResponse<TaskWithSubTasks>>;
  updateTask: (taskId: string, taskData: TaskFormData) => Promise<ServiceResponse<void>>;
...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  fetchFilteredTasks: async (userId, filter) => {
    ...
    set({
      tasks: response.success && response.data ? response.data.map(transformTaskForCard) : [],
  ...
  createTask: async (taskData, userId) => {
    ...
    if (response.success && response.data) {
      const newTask = transformTaskForCard(response.data as TaskWithSubTasks);
      set((state) => ({
    ...
    if (response.success) {
      set((state) => ({
        tasks: state.tasks.map((task) =>
          task.id === taskId
            ? transformTaskForCard({ ...task, ...taskData, id: taskId })
            : task
        ),
        error: null,
        taskToEdit: null,
    ...
    if (response.success) {
      ...
      set((state) => ({
        tasks: state.tasks.map((task) => {
          ...
          const newCompletedCount = updatedSubTasks?.filter((st: any) => st.completed).length;
          return {
            ...task,
            completed,
