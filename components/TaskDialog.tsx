...
const validateForm = (state: {
  ...
  if (state.title.length > 100) return "Title must be less than 100 characters";
  if (state.description.length > 500) return "Description must be less than 500 characters";
...
export function TaskDialog({
  ...
  const { createTask, updateTask, currentUserId, taskToEdit } = useTaskStore();
  ...
  }, [title, description, priority, deadline, subTasks]);
  ...
  const keyExtractor = useCallback((item: SubTask) => item.id ?? `temp-${Math.random()}`, []);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          alignSelf="center"
        >
          <Dialog.Title fontSize="$8" fontWeight="bold" color="$green10">
          ...
          <YStack gap="$2">
            <Text fontSize="$5" fontWeight="bold">Priority</Text>
            {priorityButtons}
          </YStack>

          <YStack gap="$2">
            <XStack justifyContent="space-between" alignItems="center">
              <Text fontSize="$5" fontWeight="bold">Deadline *</Text>
              {deadline && (
                <Button
          ...
          <YStack gap="$2">
            <XStack justifyContent="space-between" alignItems="center">
              <Text fontSize="$5" fontWeight="bold">Sub-Tasks ({subTasks.length})</Text>
              <Button
          ...
          <XStack justifyContent="flex-end" gap="$2">
            ...
            <Button
              ...
              opacity={isLoading || !title.trim() || !deadline ? 0.6 : 1}
            >
              {isLoading
                ? taskToEdit
                  ? "Updating..."
